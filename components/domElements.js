export const body = document.body;
export const todoWrapper = document.querySelector("#todoWrapper");
export const createTodoDiv = document.querySelector("#createTodo");
export const buttonCreate = document.querySelector("#buttonCreate");
export const cancelTodoButton = document.querySelector("#cancelTodo");
export const textArea = document.querySelector("#enterTodo");
export const addTodoButton = document.querySelector("#addTodo");
export const emptyNotice = document.createElement("h2");
